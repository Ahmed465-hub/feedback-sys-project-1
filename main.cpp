...
void registration()
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
                cout << "\033[31m** Name cannot be empty.please enter name. **\033[0m\n";
            }
        } while (fb.username.empty());
...
void feedback_delete(int i)
{
    ...
    system("pause");
    clearscreen();
    cout << "  Name: " << feedbacks[i].username;
    for (int j = 0; j < feedbacks[i].feedback_count; j++)
        show_feedbacks(i, j);
}
void feedback_edit(int i)
{
    ...
    do
    {
        ...
        else if (edit_opt == 5)
        {
            ...
            cout << "\n\nEditing feedback type (current: " << feedbacks[i].feedback_type[index] << "): ";
            cout << "\nChoose your feedback type :\n1. bug report\n2. feature requests\n3. other feedbacks\n=> ";
            int typechoice;
            cin >> typechoice;
            cin.ignore();
            feedbacks[i].feedback_type[index] = fb_type[typechoice];
...
void most_rep_typ()
{
    ...
    int bug_count = 0, feature_count = 0, general_count = 0;
    for (int i = 0; i < feedback_index; i++)
    {
        for (int j = 0; j < feedbacks[i].feedback_count; j++)
        {
            if (feedbacks[i].feedback_type[j] == "bug report")
                bug_count++;
            else if (feedbacks[i].feedback_type[j] == "feature requests")
                feature_count++;
            else if (feedbacks[i].feedback_type[j] == "general feedback")
                general_count++;
        }
    }
    string top_type = "bug report";

    int max_count = bug_count;
    if (feature_count > max_count)
    {
        top_type = "feature requests";
        max_count = feature_count;
    }
    if (general_count > max_count)
    {
        top_type = "general feedback";
        max_count = general_count;
    }
    cout << "Most common feedback type: " << top_type << "\033[0m (" << max_count << " times)\n";
}